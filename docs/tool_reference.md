...
Lists immediate files and directories under one repo-relative map-platform directory.
...
Returns the current configured map-platform working-tree diff generated by Git.
...
Runs the repo-local map-platform verification command in the configured checkout.
