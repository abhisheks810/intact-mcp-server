...
const tools = [
  ...
  tool(
    ...
    "Write a complete text file in map_platform with strict hash and approval guards. Requires MAP_PLATFORM_WRITE_ENABLED=true.",
    {
      ...
      expected_sha256: {
        type: "string",
        description: "Current sha256 from get_map_platform_file_metadata. Required to prevent overwriting unknown edits.",
      },
      change_request_path: {
        type: "string",
...
async function callTool(name, args = {}) {
  ...
  if (name === "create_map_platform_change_request") {
    ...
    await writeFile(filePath, content, "utf8");
    return jsonContent({ created: filePath, status: "approved-for-scoped-work" });
  }
  ...
  if (name === "write_map_platform_file") {
    ...
    const relativePath = requireString(args, "path");
    const expectedSha = requireString(args, "expected_sha256");
    ...
    const current = await getMapPlatformFileInfo(relativePath);
    if (current.sha256 !== expectedSha) {
